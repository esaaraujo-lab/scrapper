...
class PapaDownloader:
    ...
    def _download_pdf(self, url: str, output_path: str) -> bool:
        ...
        for attempt in range(RETRY_ATTEMPTS):
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if 'application/json' in resp.headers.get('content-type', ''):
                        data = resp.json()
                        if data.get('url'):
                            pdf_resp = self.session.get(f"https://portal2025.papaconcursos.com.br{quote(data['url'])}", stream=True, timeout=60)
                            if pdf_resp.status_code == 200: pdf_data = pdf_resp.content
                    elif resp.content.startswith(b'%PDF'):
                        pdf_data = resp.content
                    else:
                        pdf_resp = self.session.get(url, stream=True, timeout=60)
                        if pdf_resp.status_code == 200 and pdf_resp.content.startswith(b'%PDF'): pdf_data = pdf_resp.content
            ...
            time.sleep(2)
        with open("failed_downloads.log", "a") as f: f.write(f"{url}\n")
        return False
...
def main():
    ...
    downloader.login(
        ...
        jsessionid="0608A2717B5193D4917DF3243C94ED45",
        chave="50792781f9163e4fee8d62738ea23776198294970fc5dc857256ca9d91e4d48a23921ca909225cd2686cbfd049f0fe891b53411f6fbd5a6471e4cd03b153d6fe"
    )
