...
# Tentar importar DDownloader
try:
    from DDownloader import Downloader as DDownloaderClient
    DDOWNLOADER_AVAILABLE = True
except ImportError:
    DDOWNLOADER_AVAILABLE = False
    DDownloaderClient = None
    print("⚠ DDownloader não disponível. Usando yt-dlp nativo.")
...
KEYOS_TOKEN = ""
KEYOS_WVD_PATH = "device.wvd"
KEYOS_ENABLED = True

# Configurações DDownloader
USE_DDOWNLOADER = DDOWNLOADER_AVAILABLE  # Usar DDownloader se disponível
DDOWNLOADER_WORKERS = 4
DDOWNLOADER_RETRIES = 3

# Garanta que os diretórios existam
for d in [BASE_DIR, TEMP_DIR, CACHE_DIR, DRM_DIR, LOGS_DIR]:
    os.makedirs(d, exist_ok=True)

# Configurar logging com arquivo
log_file = LOGS_DIR / f"downloader_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
class KeyosResolver:
    ...
    def __init__(self, session: requests.Session, wvd_path: str = KEYOS_WVD_PATH, keyos_token: str = KEYOS_TOKEN):
        ...
        self.key_cache = {}
        self.api_calls = 0  # Contador de chamadas KeyOS
    ...
    def fetch_drm_key_automatically(self, mpd_url: str) -> Optional[str]:
        ...
        if not WIDEVINE_AVAILABLE or not self.keyos_token:
            logger.debug("KeyOS desabilitado ou configuração incompleta.")
            return None
        
        # Verificar cache
        if mpd_url in self.key_cache:
            logger.debug("Usando chave em cache")
        ...
        try:
            ...
            logger.info("🔐 Inicializando CDM Widevine L3...")
            ...
            logger.debug(f"Chave cacheada (cache size: {len(self.key_cache)})")
            return keys[0]
...
class DDownloaderWrapper:
    """Wrapper para usar DDownloader se disponível."""
    
    def __init__(self, output_dir: str = str(TEMP_DIR)):
        self.output_dir = output_dir
        self.available = DDOWNLOADER_AVAILABLE
        
        if self.available:
            try:
                self.client = DDownloaderClient(
                    output_dir=output_dir,
                    max_workers=DDOWNLOADER_WORKERS,
                    retry_attempts=DDOWNLOADER_RETRIES
                )
                logger.info("✓ DDownloader inicializado")
            except Exception as e:
                logger.warning(f"⚠ Erro ao inicializar DDownloader: {e}")
                self.available = False
    
    def download(self, url: str, output_path: str, progress_callback=None) -> bool:
        """Download com DDownloader se disponível, fallback para requests."""
        ...
        try:
            ...
            logger.debug(f"DDownloader falhou: {e}")
            return False

# ============================================================================
# HELPER DE DESCRIPTOGRAFIA DRM
# ============================================================================
...
class LinkCollector:
    ...
    def collect_all_links(self, course_id: str, course_name: str, force_rebuild: bool = False) -> Dict:
        ...
        logger.info(f"✓ {len(all_links)} links coletados e cacheados")
        return all_links
...
class PapaDownloader:
    def __init__(self):
        ...
        self.ddownloader = DDownloaderWrapper(str(TEMP_DIR))
    ...
    def _print_summary(self):
        ...
        if self.drm_failed_videos:
            logger.warning(f"Vídeos com DRM falho: {len(self.drm_failed_videos)}")
        logger.info(f"PDFs OK: {self.successful_pdfs}")
        logger.info(f"PDFs com falha: {self.failed_pdfs}")
        logger.info(f"Chamadas KeyOS API: {self.keyos_resolver.api_calls}")
        logger.info(f"Cache de chaves: {len(self.keyos_resolver.key_cache)}")
        logger.info(f"Tempo total: {elapsed:.1f}s ({int(elapsed//60)}m {int(elapsed%60)}s)")
        logger.info(f"{'='*70}\n")
    ...
    def _get_key_for_video(self, manifest_url: str) -> Optional[str]:
        ...
        if KEYOS_ENABLED and self.keyos_resolver:
            logger.debug("Tentando obter chave via KeyOS...")
    ...
    def _download_video(self, item_name: str, video_url: str):
        ...
        if key_pair:
            logger.info(f"  → Chave DRM: {key_pair[:20]}...")
    ...
    def _download_video_internal(self, manifest_url: str, output_path: str, key_pair: Optional[str], item_name: str):
        ...
        try:
            ...
            logger.debug(f"⚠ Método direto falhou: {e}")
        ...
        try:
            ...
            if check_binary('shaka-packager') or check_binary('packager'):
                try:
                    ...
                    logger.warning(f"   [!] shaka-packager falhou: {err}")
            ...
            if not decrypted and check_binary('mp4decrypt'):
                try:
                    ...
                    logger.warning(f"   [!] mp4decrypt falhou: {err}")
            ...
            if os.path.exists(output_path) and os.path.getsize(output_path) > 1024 * 1024:
                logger.info(f"✓ Vídeo OK (Fallback): {os.path.basename(output_path)}")
                with self.lock:
                    self.successful_videos += 1
            else:
                raise RuntimeError("Arquivo final inválido.")
    ...
    def _download_pdf(self, url: str, output_path: str) -> bool:
        """Baixa um PDF com suporte a DDownloader."""
        rel_path = os.path.relpath(output_path, str(BASE_DIR))
        temp_path = os.path.join(str(TEMP_DIR), rel_path)
        os.makedirs(os.path.dirname(temp_path), exist_ok=True)

        # Tentar com DDownloader primeiro
        if self.ddownloader.available:
            try:
                logger.debug("Tentando PDF com DDownloader...")
                if self.ddownloader.download(url, temp_path):
                    if os.path.exists(temp_path):
                        shutil.move(temp_path, output_path)
                        return True
            except Exception as e:
                logger.debug(f"DDownloader PDF falhou: {e}")
        ...
        for attempt in range(RETRY_ATTEMPTS):
            try:
                ...
                logger.debug(f"Tentativa {attempt+1} PDF: {e}")
                time.sleep(2 ** attempt)
...
def main():
    logger.info("="*70)
    logger.info("🎓 PapaSantos v2 - Downloader com KeyOS + DDownloader")
    logger.info("="*70)
